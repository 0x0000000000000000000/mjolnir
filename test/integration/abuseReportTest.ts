...
describe("Test: Reporting abuse", async () => {
    ...
    it('The redact action works', async function() {
        ...
        matrixClient().inviteUser(await moderatorUser.getUserId(), mjolnir().managementRoomId);
        await moderatorUser.joinRoom(mjolnir().managementRoomId);
        ...
        for (let button of buttons) {
            if (button["content"]["m.relates_to"]["key"].includes("[redact-message]")) {
                ...
                await moderatorUser.sendEvent(mjolnir().managementRoomId, "m.reaction", button["content"]);
                break;
            }
        }
        ...
        for (let event of notices) {
            ...
            await moderatorUser.sendEvent(mjolnir().managementRoomId, "m.reaction", event["content"]);
            break;
        }
